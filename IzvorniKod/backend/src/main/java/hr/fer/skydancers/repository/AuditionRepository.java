...
//Ovaj interface predstavlja repozitorij za audicije
@Repository
public interface AuditionRepository extends JpaRepository<Audition, Integer> {
	...
	@Query("SELECT a FROM Audition a " + "JOIN a.styles d " + "WHERE lower(trim(a.location)) = lower(trim(:location)) "
			+ "AND NOT EXISTS ( " + "    SELECT 1 FROM a.styles ds " + "    WHERE ds.name NOT IN :styles "
			+ "    AND ds.name = d.name " + ") " + "AND EXISTS ( " + "    SELECT 1 FROM a.styles ds "
			+ "    WHERE ds.name IN :styles " + "    AND ds.name = d.name " + ")")
